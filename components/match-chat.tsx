      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {latestComment ? (
            ...
            <p className="text-xs text-white/25 mt-0.5">Bấm để mở chat — hãy là người đầu tiên!</p>
          )}
        </div>
        <ChevronUp size={16} className="text-white/20 flex-shrink-0" />
      </div>
    </button>
  )
}
      ...
      <div ref={listRef} className="overflow-y-auto hide-scrollbar px-3 py-2 space-y-0.5"
        ...
        {comments.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-white/15">
            ...
            <p className="text-xs mt-1 text-white/20">Nói gì đó đi, đừng ngại 😏</p>
          </div>
        ) : (
          comments.map((c, i) => {
